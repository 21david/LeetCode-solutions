 ...
 */
/* 
After seeing editorial a few days ago:
...
function lcaDeepestLeaves(root: Node): Node {
    const dfs: DFS = (node) => {
        if (node === null)
            return [0, null];
        
        let [left_depth, left_lca] = dfs(node.left);
        let [right_depth, right_lca] = dfs(node.right);
